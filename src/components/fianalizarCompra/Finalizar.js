...
const ModalShow = createContext(false)
...
export function FinalizarCompra(){
    ...
    const onClickFinalizar = (e) => {
        e.preventDefault()
        if(
            cliente.celular.length &&
            cliente.cep.length &&
            cliente.cidade.length &&
            cliente.cpf.length &&
            cliente.email.length &&
            cliente.endereco.length &&
            cliente.estado.length &&
            cliente.nome.length
        ){
            console.log(cliente.celular.length,
                cliente.cep.length,
                cliente.cidade.length,
                cliente.cpf.length,
                cliente.email.length,
...
function ModalReposta(props) {
    const {setModal} = useContext(ModalShow)
    const onClick= (e) => {
        window.location.replace('/')
    }
    const onClose = (e) => {
        setModal(lastData => ({...lastData, show: false}))
    }
    return (
      <Modal
        show={props.show}
        ...
        <Modal.Body>
            
            {
                ...
                <div className="card-fim">
                    ...
                    <Button onClick={onClose} className='m-3 text-center'>
                        Fechar
                    </Button>
